...
def init_tree(start,end,node):
    if start==end:
        ...
        return tree[node]
    mid=(start+end)//2
    tree[node]=init_tree(start,mid,node*2)*init_tree(mid+1,end,node*2+1)%MOD
    return tree[node]
...
def find_mul(s,e,n,l,r):
    if l>e or r<s:
        return 1
    if l<=s and e<=r:
        return tree[n]
    mid=(s+e)//2
    return (find_mul(s,mid,n*2,l,r)*find_mul(mid+1,e,n*2+1,l,r))%MOD
len_tree=len(number)
...
for _ in range(M+K):
    ...
    if a==1:
        ...
        print(find_mul(0,len_tree-1,1,b-1,c-1))
