...
1. 사용자로부터 받은 입력을 2차원 리스트의 형태로 저장 e.g. [['C','A','A','B'],['A','D','C','B']] 
2. queue에 현재 x,y좌표와 x,y좌표에서의 board의 알파벳을 set의 형태로 집어넣는다. 
3. queue의 처음 들어온 x,y,alphabet을 x,y,ans 형태로 꺼낸 뒤 상, 하, 좌, 우로 이동해서 전체 범위 안에 있는지 확인 
